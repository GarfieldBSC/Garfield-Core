...
contract Garfield is ERC20, Ownable {
   ...
   GFLDDividendTracker public dividendTracker;

    address private liquidityWallet;

    uint256 public maxSellTransactionAmount = 1000000000000000 * (10**9);
    uint256 public swapTokensAtAmount = 5000000000000 * (10**9);
    ...
    constructor() public ERC20("Garfield", "$GFLD") {
        ...
        _marketingWallet = 0x6eef861cD5f9359bcA88Bc61fbfCD1256E651C31;
    	dividendTracker = new GFLDDividendTracker();

    	liquidityWallet = owner();

    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x519d743CE097734f07CF75FccC82280f8c528B2E);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GFLD: The dividend tracker already has that address");
        GFLDDividendTracker newDividendTracker = GFLDDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "GFLD: The new dividend tracker must be owned by the Garfield contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GFLD: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GFLD: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GFLD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GFLD: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "GFLD: The liquidity wallet is already this address");
        excludeFromFees(newLiquidityWallet, true);
        emit LiquidityWalletUpdated(newLiquidityWallet, liquidityWallet);
        liquidityWallet = newLiquidityWallet;
    }
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GFLD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GFLD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
	    ...
	    require(tradingOpen || _isExcludedFromFees[from],"Trading not open yet");
        require(amount <= _maxTxAmount || _isExcludedFromFees[from], "Transfer amount exceeds the maxTxAmount.");
...
contract GFLDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("GFLD_Dividend_Tracker", "GFLD_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**15); //must hold 1000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "GFLD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "GFLD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GFLD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GFLD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GFLD_Dividend_Tracker: Cannot update claimWait to same value");
